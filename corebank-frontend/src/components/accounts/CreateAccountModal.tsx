...
interface CreateAccountModalProps {
  ...
  onClose: () => void
  onSuccess?: () => void
  isInFlow?: boolean // Whether this modal is part of the onboarding flow
}

export default function CreateAccountModal({ isOpen, onClose, onSuccess, isInFlow = false }: CreateAccountModalProps) {
  ...
  const handleClose = () => {
    if (!isCreating && !isInFlow) {
      // Reset form when closing (but not during onboarding flow)
      setAccountType('checking')
      setInitialDeposit('')
      onClose()
    }
  ...
  // If this is part of the onboarding flow, render without backdrop and positioning
  if (isInFlow) {
    return (
      <div className="w-full max-w-md mx-auto">
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          {/* Header */}
          <div className="mb-6">
            <h3 className="text-lg font-semibold text-gray-900">创建您的第一个账户</h3>
            <p className="text-sm text-gray-600 mt-1">选择账户类型并设置初始存款</p>
          </div>
          ...
          {createErrorMessages.length > 0 && (
            <div className="mb-4 rounded-lg bg-red-50 border border-red-200 p-4">
              <div className="flex items-start">
                ...
                <div>
                  <h3 className="text-sm font-medium text-red-800">创建账户失败</h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500">
                {accountType === 'checking' ? '用于日常交易和支付' : '用于储蓄和理财'}
              </p>
            </div>
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500">留空则创建零余额账户</p>
            </div>
            ...
            <button
              type="submit"
              disabled={isCreating}
              className="w-full px-4 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:bg-blue-400 disabled:cursor-not-allowed flex items-center justify-center"
            >
              {isCreating ? (
                <>
